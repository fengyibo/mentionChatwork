...
class ChatWorkListener < Redmine::Hook::Listener
  def controller_issues_new_after_save(context={})
    ...
    if issue.tracker_id.to_s === Setting.plugin_mentionChatwork["trigger_invoice_tracker"].to_s
  ...
  def controller_issues_edit_after_save(context={})
    ...
    return if disabled
    return unless room and Setting.plugin_mentionChatwork["post_updates"] == '1'
    return if issue.is_private?
  ...
  def controller_wiki_edit_after_save(context = {})
    return unless Setting.plugin_mentionChatwork["post_wiki_updates"] == '1'
